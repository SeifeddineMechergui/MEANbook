...
app.get("/books/:id", (req, resp) => {
    ...
    book_model_1.default.findById(bookId)
        .then((book) => {
        if (book) {
            resp.send(book);
        }
...
app.put("/books/:id", (req, resp) => {
    const bookId = req.params.id;
    book_model_1.default.findByIdAndUpdate(bookId, req.body)
        .then(() => {
...
app.delete("/books/:id", (req, resp) => {
    const bookId = req.params.id;
    book_model_1.default.findByIdAndRemove(bookId)
        .then((book) => {
        if (book) {
...
app.get("/pbooks", (req, resp) => {
    ...
    book_model_1.default.paginate({}, { page: p, limit: size })
        .then((result) => {
        resp.send(result);
    })
        .catch((err) => {
...
app.get("/books-search", (req, resp) => {
    ...
    book_model_1.default.find({ title: { $regex: new RegExp(".*" + keyword + ".*", "i") } })
        .then((books) => {
        if (books.length > 0) {
            resp.send(books);
        }
